...
export default function CardDrawer({
  ...
  return (
    ...
    >
      ...
      {column?.chat && (
        <div className="flex flex-col gap-3">
          {card.messages.length === 0 && !rodando && (
            <p className="text-muted-foreground text-xs">A conversa começa quando o card chega aqui.</p>
          )}
      ...
      {!column?.chat && card.messages.length > 0 && (
        <Collapsible
          open={arquivoAberto}
      ...
      {(!column?.chat || card.history.length > 0) && (
